...
export const projects: Project[] = [
  {
    ...
    description:
      "A dual-channel AI agent — voice telephony + web chat — that answers recruiter questions about your skills, projects, and experience in real time. Built on a RAG pipeline grounded in resume and GitHub data, with autonomous interview scheduling via Cal.com.",
    highlights: [
      "RAG pipeline with 3072-dim OpenAI embeddings indexed in Pinecone — 98.2% groundedness on a 33-pair golden Q&A set scored by a GPT-4o judge.",
      "Voice pipeline via Vapi · Deepgram Nova-3 STT at 99.1% accuracy · GPT-4o Mini at ~390ms latency · P95 response 1.85s, under the 2.0s SLA.",
      "End-to-end Cal.com booking with 6/6 confirmed test bookings — hardened against timezone offset, slot-starvation and webhook routing failures.",
    ],
  ...
  },
  {
    ...
    highlights: [
      ...
      "Answer Confidence Scoring combines Gemini 2.5 Flash self-assessment with vector similarity into an auditable confidence percentage per response.",
    ],
  ...
  },
  {
    ...
    highlights: [
      ...
      "Automated TDD self-correction loop (Pytest/Jest) — lifted generation success from 76% → 96%.",
  ...
  },
  {
    ...
    highlights: [
      "AI course generation via Groq SDK (Llama 3.3 70B) — structured JSON lessons, summaries and 5-question MCQs, auto-embedding top YouTube resources per module.",
  ...
  },
  {
    ...
    description:
      "A custom-engineered registration and ticketing platform for Plinth — LNMIIT's annual national techno-cultural festival. Multi-member group registrations, real-time GST-inclusive pricing, referral discounts, UPI payments and Cloudinary-backed proof uploads — over a two-stage transactional commit architecture that prevents orphaned payments.",
    highlights: [
      "Two-stage transactional commit — booking persists with a unique bookingId before payment begins, guaranteeing reconcilability even if the browser closes mid-upload.",
      "Client-side real-time pricing engine — base passes, per-member event add-ons, multi-day pass rates, referral deductions and 18% GST computed live before the UPI QR renders.",
      "Cloudinary direct-upload pipeline — payment proof bypasses the Express backend, storing only the CDN URL in MongoDB to dodge Vercel payload limits.",
    ],
  ...
  },
  {
    ...
    description:
      "A React 19 + Vite 7 web app that replaces standard Minimax — which evaluates up to 362,880 board states recursively — with a custom deterministic heuristic decision tree that resolves every move in true O(1) time.",
    highlights: [
      "O(1) deterministic AI covering diagonal forks, double-edge traps and corner-opening counters — guaranteed win or forced draw across every game state.",
